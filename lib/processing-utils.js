...
} from "./formula-detection.js";

export function calculateCropPixels(rect, imageWidth, imageHeight) {
  if (rect.coordinateSpace === "image-pixels") {
    const sx = clamp(Math.round(rect.x), 0, imageWidth);
    const sy = clamp(Math.round(rect.y), 0, imageHeight);
    const right = clamp(
...
export function normalizeOCRText(text) {
  ...
  const { protected: protectedText, tokens } = protectFormulaPlaceholders(normalized);

  const cleaned = normalizeOCRChoiceMarkers(protectedText)
    .replace(/\b(?:C[oôơãaâ]u|Cau|Cãu|Côu)\s*([.!:!]?)\s*(\d{1,2})/giu, "Câu $2")
    .replace(/\b(?:C[oôơãaâ]u|Cau|Cãu|Côu)\b/giu, "Câu")
    .replace(/\bcóc\b/giu, "các")
    .replace(/\bphỏi\b/giu, "phải")
    .replace(/\btach\b/giu, "tách")
    .replace(/\bthiết\s*lập\b/giu, "thiết lập")
    .replace(/[ \t]+/g, " ")
    .replace(/ *\n */g, "\n")
    .replace(/\n{3,}/g, "\n\n")
    .trim();

  return restoreFormulaPlaceholders(cleaned, tokens);
}
...
export function parseAIResult(
  ...
  const fallbackQuestion = {
    ...
    confidence: "low",
    shortExplanation: "Could not parse the AI response.",
    coreKnowledge: "",
  ...
  try {
    ...
    const result = parseAIResult(
      ...
      ocrText,
      { requestedMode, userSelectedAnswer, trustModelSelections }
    );
    return {
      ...result,
...
function normalizeParsedQuestion(
  ...
  let questionLineRefs =
    providedLineRefs.length > 0
      ? providedLineRefs
      : inferredLineScopes[index] || [];
  const inferredLineRefs = inferredLineScopes[index] || [];
  if (
    providedLineRefs.length > 0 &&
    inferredLineRefs.length > providedLineRefs.length
  ) {
    const providedScopeText = getQuestionScopeText(
      numberedOCR.lines,
      providedLineRefs,
      questionText
    );
    const inferredScopeText = getQuestionScopeText(
      numberedOCR.lines,
      inferredLineRefs,
      questionText
    );
    if (
      extractVisibleOptions(providedScopeText).length < 2 &&
      extractVisibleOptions(inferredScopeText).length >= 2
    ) {
      questionLineRefs = inferredLineRefs;
    }
  }
  const scopedOCRText =
    getQuestionScopeText(
  ...
  const requiredAnswerCount =
    normalizeRequiredAnswerCount(parsed?.requiredAnswerCount) ??
    detectRequiredAnswerCount(scopedOCRText);
...
function parseJSONResponse(content) {
  const json = extractJSONObject(stripThinkingBlocks(content));
  try {
    return requireAIResultObject(JSON.parse(json));
  } catch (initialError) {
    const repaired = repairCommonJSONErrors(json);
    if (repaired === json) {
      throw initialError;
    }
    return requireAIResultObject(JSON.parse(repaired));
  }
}

function requireAIResultObject(value) {
...
function recoverPartialAIResult(content) {
  const source = stripThinkingBlocks(content);
  const recoveredQuestions = recoverCompleteQuestionObjects(source);
  if (recoveredQuestions.length) {
    return {
      mode: extractJSONStringField(source, "mode") || "learning",
...
function recoverCompleteQuestionObjects(source) {
  const questionsMatch = /["']?questions["']?\s*:\s*\[/i.exec(source);
  if (!questionsMatch) {
    return [];
  }
  ...
  for (let index = startIndex; index < source.length; index += 1) {
    ...
    if (depth === 0 && objectStart >= 0) {
      try {
        const objectText = repairCommonJSONErrors(
          source.slice(objectStart, index + 1)
        );
        questions.push(requireAIResultObject(JSON.parse(objectText)));
      } catch {
        // Skip malformed question objects and keep scanning later objects.
      }
      objectStart = -1;
    }
  }
...
function normalizeOptionAnalysis(
  value,
  ocrText,
  { trustModelSelections = false } = {}
) {
  ...
  if (trustModelSelections && visibleOptions.length < 2) {
    return value
      .map((option) => ({
...
function normalizeAnswerSelections(
  ...
  const addSummary = (candidate) => {
    if (candidate !== null && candidate !== undefined && candidate !== "") {
      summaryCandidates.push(candidate);
    }
  };

  extractCorrectAnswerFromEvaluation(parsed, visibleOptions).forEach(
    addPriority
  );
  ...
  extractAnswerLabels(rawAnswerText, visibleOptions).forEach(addSummary);

  if (Array.isArray(parsed?.answerSelections)) {
    parsed.answerSelections.forEach(addSelection);
  }
  if (Array.isArray(parsed?.answers)) {
    parsed.answers.forEach(addSelection);
  }
  ...
  // Normalize explicit candidates to see if we have a single, clear answer.
  const prioritySelections = normalizeAnswerCandidateList(
    priorityCandidates,
    visibleOptions,
    ocrText,
    trustModelSelections
  );
  const selectionSelections = normalizeAnswerCandidateList(
    selectionCandidates,
    visibleOptions,
    ocrText,
    trustModelSelections
  );
  const summarySelections = normalizeAnswerCandidateList(
    summaryCandidates,
    visibleOptions,
    ocrText,
    trustModelSelections
  );
  const explicitSelections = normalizeAnswerCandidateList(
    explicitCandidates,
    visibleOptions,
    ocrText,
    trustModelSelections
  ...
  const analysisSelections = normalizeAnswerCandidateList(
    analysisCandidates,
    visibleOptions,
    ocrText,
    trustModelSelections
  ...
  const isMultiSelect =
    ...
    (requiredAnswerCount === null && hasMultipleExplicit && !isTrueFalse);

  let selections = [];
  let expandedFromOption = false;

  if (!isMultiSelect) {
    // For single-select questions, prefer corrected feedback, then the
    // structured answerSelections field. answerLabel/answerText are only a
    // compatibility summary and may contradict the structured answer.
    if (prioritySelections.length > 0) {
      selections = [prioritySelections[0]];
    } else if (selectionSelections.length === 1) {
      selections = selectionSelections;
    } else if (selectionSelections.length > 1 && isTrueFalse) {
      selections = [selectionSelections[0]];
    } else if (summarySelections.length === 1) {
...
function normalizeAnswerCandidateList(
  candidates,
  visibleOptions,
  ocrText,
  trustModelSelections = false
...
function normalizeAnswerSelection(
  candidate,
  visibleOptions,
  ocrText,
  trustModelSelections = false
  ...
  const label = resolveVisibleOptionLabel(rawLabel, rawText, visibleOptions);
...
function resolveVisibleOptionLabel(rawLabel, rawText, visibleOptions) {
  const exactLabel = cleanField(rawLabel).toUpperCase();
  if (visibleOptions.some((option) => option.label === exactLabel)) {
    return exactLabel;
  }

  const labelPrefix =
    extractLeadingOptionLabel(rawLabel, visibleOptions) ||
    extractLeadingOptionLabel(rawText, visibleOptions);
  return labelPrefix || "";
}

function extractLeadingOptionLabel(value, visibleOptions) {
  const source = cleanField(value).toUpperCase();
  if (!source || visibleOptions.length < 2) {
    return "";
  }

  const match = source.match(/^([A-Z0-9]{1,3})\s*[.):\-]\s*\S/);
  const candidate = match?.[1] || "";
  return visibleOptions.some((option) => option.label === candidate)
    ? candidate
    : "";
}
...
function extractCorrectAnswerFromEvaluation(parsed, visibleOptions) {
  if (visibleOptions.length < 2) {
    return [];
  }

  const evaluation = parsed?.userAnswerEvaluation;
  if (!evaluation || typeof evaluation !== "object") {
    return [];
  }

  const source = removeDiacritics(
    [
      evaluation.feedback,
  ...
  if (!label || !visibleOptions.some((option) => option.label === label)) {
    return [];
  }
...
function normalizeRequiredAnswerCount(value) {
