...
function normalizeAnswerSelections(
  ...
  if (Array.isArray(parsed?.answerSelections)) {
    parsed.answerSelections.forEach(addCandidate);
  }
  if (Array.isArray(parsed?.answers)) {
    parsed.answers.forEach(addCandidate);
  }
  ...
  extractAnswerLabels(rawAnswerText, visibleOptions).forEach(addCandidate);
  if (!candidates.length || (rawAnswerText && candidates.length <= 1)) {
    addCandidate({
      label: normalizeAnswerLabel(
        parsed?.answerLabel,
        legacyAnswer,
        ocrText
      ),
  ...
  for (const candidate of candidates) {
    ...
    if (!duplicate) {
      ...
      expandedFromOption ||= normalized.expandedFromOption;
    }
  }

  return { selections, expandedFromOption };
}
