...
export function calculateOverallReliability({
  ...
  if (hasOCRConfidence) {
    ...
    reasons.push("Image-direct mode reads the crop without local OCR.");
  ...
  reasons.push(
    `${provider === "openai" ? "API model" : "Local AI"} confidence is ${normalizedAIConfidence}.`
  );
  ...
  if (parseStatus !== "parsed") {
    reasons.push(
      parseStatus === "recovered"
        ? "The API response was truncated or malformed; complete question results were recovered."
        : "The AI response could not be parsed reliably."
    );
    level = "low";
